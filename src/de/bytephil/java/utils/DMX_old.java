...
public class DMX_old {
    ...
    public static void interruptServiceRoutine1() {
        ...
        executor.scheduleAtFixedRate(DMX_old::test,0,period,TimeUnit.MICROSECONDS);
    }
        ...
        while (processed <= 4096) {
           ...
           // Output.outputDMX(BitType.ZERO);
            ...
            for (int i = 0; i < 8; i++) {
                ...
                futures.add(scheduler.schedule(() -> 0, 4, TimeUnit.MICROSECONDS));                                 // Bit-Time = 4 us
                for (Future<Integer> e : futures) {e.get();}
                futures.clear();
